...
class RDWWindow:
    ...
    def render(self):
        ...
        for ag, ag_color in self.agents:
            if ag.enable_render:
                # self.render_queue.append(
                #     (self.ui_font.render(f'name:{ag.name}', True, self.back_clr),
                #      (945, 85 + v_obs_width)))
                # self.render_queue.append(
                #     (self.ui_font.render(f'resets:{ag.resetter.reset_num}', True, self.back_clr),
                #      (945, 115 + v_obs_width)))
                # self.render_queue.append(
                #     (self.ui_font.render(f'rdw rate:{ag.rdw_rate:.4f}', True, self.back_clr),
                #      (945, 145 + v_obs_width)))

                self.render_queue.append(
                    (self.ui_font.render(f'gt:{ag.gainer.g_values[0]:.4f}', True, self.back_clr),
                     (945, 85 + v_obs_width)))
                self.render_queue.append(
                    (self.ui_font.render(f'gr:{ag.gainer.g_values[1]:.4f}', True, self.back_clr),
                     (945, 115 + v_obs_width)))
                self.render_queue.append(
                    (self.ui_font.render(f'gc:{1 / ag.gainer.g_values[2]:.4f}', True, self.back_clr),
                     (945, 145 + v_obs_width)))
                self.render_queue.append(
                    (self.ui_font.render(f'reset:{ag.resetter.reset_num:.4f}', True, self.back_clr),
                     (945, 175 + v_obs_width)))
