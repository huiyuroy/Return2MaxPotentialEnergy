...
class TurnArcResetter(BaseResetter):
    """
    Proposed by https://ieeexplore.ieee.org/document/9382909
    """

    def __init__(self):
        super().__init__()
        self.reset_type = 'TurnArc'
        self.init_fwd = [0, 1]
        self.theta_range = tuple(map(lambda x: x * DEG2RAD, range(-180, 180, 2)))

    def calc_reset_target_fwd(self):
        target_fwd = None
        min_loss = float('inf')
        d_vir_fwd_inter, d_vir_fwd = geo.calc_ray_poly_intersection(self.v_loc, self.v_fwd, self.v_scene.poly_contour)
        self.reset_target_fwd = None

        for theta in self.theta_range:
            pot_fwd = geo.rot_vecs(self.init_fwd, theta)
            nx_pot_loc = self.p_loc + pot_fwd * self.reset_pred_t
            if self.p_scene.poly_contour_safe.covers(Point(nx_pot_loc)):
                d_phy_theta_inter, d_phy_theta = geo.calc_ray_poly_intersection(self.p_loc, pot_fwd,
                                                                                self.p_scene.poly_contour)
                if d_phy_theta > d_vir_fwd:
                    target_fwd = pot_fwd
                    break
                else:
                    cur_loss = np.abs(d_phy_theta - d_vir_fwd)
                    if cur_loss < min_loss:
                        min_loss = cur_loss
                        target_fwd = pot_fwd
        self.reset_target_fwd = target_fwd
