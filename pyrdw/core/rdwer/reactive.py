...
class ARCRdwManager(SteerRdwManager):
    """
    Refer to https://ieeexplore.ieee.org/document/9382909

    https://gamma.umd.edu/researchdirections/vrlocomotion/arc/

    """

    def __init__(self):
        super(ARCRdwManager, self).__init__()
        self.rdw_type = 'ARC Rdw'

    def prepare(self):
        super().prepare()

    def calc_vir_state(self):
        assert isinstance(self.agent.gainer,
                          pyrdw.core.gain.base.ARCGainManager), 'must set ARCGainManager to ARCRDWManager'
        p_fwd_vec, p_fwd_vel, p_ver_vec, p_ver_vel = self.p_vel_decompose()

        if self.enable_rdw:
            # 当前正在行走
            if p_fwd_vel > self.steer_vel_dt * self.time_step:
                vir_rot_vel = self.p_rot + p_fwd_vel * 0.01 * self.agent.gainer.g_values[2]  # 基于行走速度的方向偏转
            else:
                vir_rot_vel = self.p_rot * self.agent.gainer.g_values[1]
            vir_fwd_vel = p_fwd_vel * self.agent.gainer.g_values[0]
            vir_ver_vel = p_ver_vel
        else:
            vir_rot_vel = self.p_rot
            vir_fwd_vel = p_fwd_vel
